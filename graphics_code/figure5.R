...
g3 = ggplot(subset(err, type=="4d")) + 
  geom_errorbar(aes(x=id, ymin=mean-2*se, ymax=mean+2*se), size=0.1) + 
  geom_point(aes(id, mean), size=0.5) + 
  theme_ipsum() + 
  labs(y="u(d)", x=NULL, title="Three design points") 

g4 = ggplot(subset(err, type=="5d")) + 
  geom_errorbar(aes(x=id, ymin=mean-2*se, ymax=mean+2*se), size=0.1) + 
  geom_point(aes(id, mean), size=0.5) + 
  theme_ipsum() + 
...
tmp = subset(err, type=="Four design points")
head(tmp)
g
res = 600
png("graphics/figure5.png", width=8*res, height=5*res, res=res)
print(g)
dev.off()
